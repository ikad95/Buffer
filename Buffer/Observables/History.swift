...
class History { // swiftlint:disable:this type_body_length
  ...
  let logger = Logger(label: "org.codexvault.Buffer")
  ...
  func load() async throws {
    ...
    // Rebuild search index on load
    Task.detached(priority: .background) {
      Search.rebuildIndex(items: await self.all)
    }
  ...
  private func limitHistorySize(to maxSize: Int) {
    // Skip if unlimited history is enabled
    guard !Defaults[.unlimitedHistory] else { return }
    ...
    // Apply LRU eviction based on age if enabled
    if Defaults[.enableLRU] {
      evictOldItems()
    }
  }
  ...
  private func evictOldItems() {
    let keepDays = Defaults[.lruKeepDays]
    guard keepDays > 0 else { return }

    let cutoffDate = Calendar.current.date(byAdding: .day, value: -keepDays, to: Date.now) ?? Date.now
    let oldItems = all.filter { item in
      item.isUnpinned && item.item.lastCopiedAt < cutoffDate
    }

    oldItems.forEach(delete)
  }
  ...
  func add(_ item: HistoryItem) -> HistoryItemDecorator {
    ...
    Search.indexItem(itemDecorator)
