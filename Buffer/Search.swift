...
class Search {
  enum Mode: String, CaseIterable, Identifiable, CustomStringConvertible, Defaults.Serializable {
    ...
    case regexp
    case mixed
    case fulltext // FTS5 full-text search
    ...
    var description: String {
      ...
      case .fulltext:
        return NSLocalizedString("Full-Text", tableName: "GeneralSettings", comment: "")
      }
    }
  }
  ...
  func search(string: String, within: [Searchable]) -> [SearchResult] {
    ...
    case .fulltext:
      return fulltextSearch(string: string, within: within)
    default:
  ...
  private func fulltextSearch(string: String, within: [Searchable]) -> [SearchResult] {
    let ftsResults = SearchIndexManager.shared.search(query: string)

    // Map FTS results back to Searchable items
    let itemMap = Dictionary(uniqueKeysWithValues: within.map { ($0.id.uuidString, $0) })

    return ftsResults.compactMap { ftsResult in
      guard let item = itemMap[ftsResult.itemId] else { return nil }

      // Find highlight ranges in the title
      var ranges: [Range<String.Index>] = []
      if let range = item.title.range(of: string, options: .caseInsensitive) {
        ranges.append(range)
      }

      return SearchResult(score: ftsResult.score, object: item, ranges: ranges)
    }
  }

  // MARK: - Index Management

  /// Index an item for full-text search
  static func indexItem(_ item: HistoryItemDecorator) {
    SearchIndexManager.shared.indexItem(
      id: item.id.uuidString,
      title: item.title,
      content: item.item.previewableText,
      application: item.item.application
    )
  }

  /// Remove an item from the search index
  static func removeFromIndex(_ item: HistoryItemDecorator) {
    SearchIndexManager.shared.removeItem(id: item.id.uuidString)
  }

  /// Rebuild the entire search index
  static func rebuildIndex(items: [HistoryItemDecorator]) {
    SearchIndexManager.shared.clearIndex()
    for item in items {
      indexItem(item)
    }
    SearchIndexManager.shared.optimize()
  }
