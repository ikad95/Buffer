...
class HistoryTests: XCTestCase {
  ...
  func testAddingItemThatIsSupersededByExisting() {
    ...
    firstItem.application = "Buffer.app"
    ...
    secondItem.application = "Buffer.app"
  ...
  func testAddingItemFromBuffer() {
    ...
    let secondContents = [
      ...
      HistoryItemContent(
        type: NSPasteboard.PasteboardType.fromBuffer.rawValue,
        value: "".data(using: .utf8)
      )
    ]
    let second = HistoryItem()
    Storage.shared.context.insert(second)
    second.application = "Buffer.app"
